...
#include <iomanip>
#include <string>
#include "idealMHD_2D_symmetric_y.hpp"


void IdealMHD2DSymmetricY::initializeU(
...
void IdealMHD2DSymmetricY::oneStepRK2()
{
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            bxOld[i][j] = U[4][i][j];
    ...
    //これはどうにかすること。保守性が低い
    boundary.symmetricBoundary2ndY(UBar);
    ...
    //これはどうにかすること。保守性が低い
    boundary.symmetricBoundary2ndY(U);
}


void IdealMHD2DSymmetricY::save(
...
void IdealMHD2DSymmetricY::calculateDt()
{
...
bool IdealMHD2DSymmetricY::checkCalculationIsCrashed()
{
    for (int comp = 0; comp < 8; comp++) {
        for (int i = 0; i < nx; i++) {
            for (int j = 0; j < ny; j++) {
...
void IdealMHD2DSymmetricY::shiftUToCenterForCT(
...
void IdealMHD2DSymmetricY::backUToCenterHalfForCT(
    std::vector<std::vector<std::vector<double>>>& U
)
{
    for (int i = 0; i < nx; i++) {
...
// getter
std::vector<std::vector<std::vector<double>>> IdealMHD2DSymmetricY::getU()
{
    return U;
}
