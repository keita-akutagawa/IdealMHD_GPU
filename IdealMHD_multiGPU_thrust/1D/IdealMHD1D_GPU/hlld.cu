...
HLLD::HLLD(int localSize)
    : localSize(localSize), 

      calculateHalfQ(localSize), 
      dQCenter(localSize),
      dQLeft(localSize),
      dQRight(localSize),
      hLLDParameter(localSize),

      flux(localSize),
      fluxOuterLeft(localSize),
      fluxMiddleLeft(localSize),
      fluxInnerLeft(localSize),
      fluxOuterRight(localSize),
      fluxMiddleRight(localSize),
      fluxInnerRight(localSize)
{
}
...
)
{
    ...
    thrust::transform(
        ...
        tupleForFluxIterator + localSize, 
        flux.begin(), 
