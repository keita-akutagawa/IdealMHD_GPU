...
)
{
    ...
    auto tupleForLeft = thrust::make_tuple(dQ.begin() - localSizeY, dQ.begin(), dQ.begin() + localSizeY);
    auto tupleForLeftIterator = thrust::make_zip_iterator(tupleForLeft);

    thrust::transform(
        tupleForLeftIterator + localSizeY, 
        tupleForLeftIterator + localSizeX * localSizeY - localSizeY, 
        dQLeft.begin() + localSizeY,
...
)
{
    ...
    thrust::transform(
        ...
        tupleForLeftIterator + localSizeX * localSizeY - 1, 
        dQLeft.begin() + 1,
        LeftParameterFunctor()
    );
}
...
)
{
    ...
    auto tupleForRight = thrust::make_tuple(dQ.begin(), dQ.begin() + localSizeY, dQ.begin() + 2 * localSizeY);
    auto tupleForRightIterator = thrust::make_zip_iterator(tupleForRight);

    thrust::transform(
        tupleForRightIterator, 
        tupleForRightIterator + localSizeX * localSizeY - 2 * localSizeY, 
        dQRight.begin(),
...
)
{
    ...
    thrust::transform(
        tupleForRightIterator, 
        tupleForRightIterator + localSizeX * localSizeY - 2, 
        dQRight.begin(),
        RightParameterFunctor()
    );
}
